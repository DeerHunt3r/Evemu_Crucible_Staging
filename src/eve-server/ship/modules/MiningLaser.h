...
protected:
    // Runs mining resolution. abort=true means partial-cycle mining.
...
private:
    ...
    // Arm/disarm mining so we do NOT mine on the immediate Activate()->DoCycle() call.
    // First real deposit happens when the first full cycle completes (timer tick).
